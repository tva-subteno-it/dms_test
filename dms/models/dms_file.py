...
class DMSFile(models.Model):
    ...
    res_model = fields.Char(
        ...
        related="directory_id.res_model"
    )
    res_id = fields.Integer(
        string="Linked attachments record ID",
        related="directory_id.res_id"
    )
    ...
    def _check_name(self):
        for record in self:
            ...
            if files.filtered(lambda file: file.name == record.name and file != record):
    ...
    def action_migrate(self, should_logging=True):
        ...
        for dms_file in self:
            if should_logging:
                _logger.info(_("Migrate File %(index)s of %(record_count)s [ %(dms_file_migration)s ]",
                               index=index,
    ...
    def check_access_rule(self, operation):
        self.directory_id.check_access_rule(operation)
    ...
    def check_access_token(self, access_token=False):
        ...
        items = self.env["dms.directory"].sudo().search([("access_token", "=", access_token)])
        if items:
            item = items[0]
    ...
    @api.model
    def _get_content_initial_vals(self):
    ...
    def search_panel_select_range(self, field_name, **kwargs):
        ...
        if field_name == "directory_id":
            ...
            if self.env.context.get("active_model", False) == "dms.directory":
                ...
                files = self.env["dms.file"].search([("directory_id", "child_of", active_id)])
                all_directories = files.directory_id + files.directory_id.parent_id
        ...
        return super(DMSFile, self.with_context(**context)).search_panel_select_range(
            field_name, **kwargs
        )
    ...
    def _inverse_content(self):
        ...
        for record in self:
            values = self._get_content_initial_vals()
            binary = base64.b64decode(record.content or "")
